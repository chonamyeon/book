  ...
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans leading-relaxed pb-20">
      ...
      <main className="w-full pt-20">
                    ...
                    return (
                      <div key={i} className={`flex ${isJames ? 'flex-row' : 'flex-row-reverse'} items-start gap-3`}>
                        <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl overflow-hidden flex-shrink-0 shadow-sm border-2 border-white bg-white">
                          <img
                            src={isJames ? '/images/celebrities/james.jpg' : '/images/celebrities/stella.jpg'}
        ...
        {/* Related Books */}
        {(() => {
          const related = adsenseBooks.filter(b => b.id !== book.id && b.category === book.category).slice(0, 3);
          if (related.length === 0) return null;
          return (
            <div className="mt-20 border-t border-gray-100 pt-16">
              <h2 className="text-xl font-bold text-gray-900 mb-8 px-2 flex items-center gap-2">
                 당신을 위한 다른 통찰력
              </h2>
              <div className="grid grid-cols-1 gap-6">
                {related.map(r => (
                  <Link key={r.id} to={`/story/${r.id}`} className="group flex gap-6 p-6 bg-white border border-gray-100 rounded-2xl hover:border-amber-200 hover:shadow-xl transition-all duration-300">
                    <div className={`w-16 h-24 flex-shrink-0 rounded-lg shadow-lg flex items-center justify-center bg-gradient-to-br ${r.gradient || 'from-gray-700 to-black'} transition-transform group-hover:scale-105`}>
                      <span className="material-symbols-outlined text-white text-2xl">{r.icon || 'book'}</span>
                    </div>
                    <div className="flex-1 py-1">
                      <h3 className="font-extrabold text-gray-900 group-hover:text-amber-600 transition-colors">{r.title}</h3>
                      <p className="text-sm text-gray-500 mt-1 font-medium">{r.author}</p>
                      <p className="text-sm text-gray-400 mt-3 line-clamp-2 leading-relaxed">{r.desc}</p>
                    </div>
                  </Link>
                ))}
              </div>
