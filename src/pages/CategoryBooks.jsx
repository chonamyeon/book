...
const CATEGORY_EDITORIAL = {
    SELF_DEV: {
        title: "자기계발 — Whiteboard의 관점",
        body: `우리는 왜 자기계발서를 읽는가? 더 나은 내일을 원하기 때문이다. 그러나 서점의 자기계발 코너를 채운 수천 권의 책 가운데 실제로 삶을 바꾸는 책은 손에 꼽는다. Whiteboard가 자기계발 카테고리에서 도서를 선정하는 기준은 단 하나다. "읽고 나서 오늘 당장 행동이 달라지는가?"

성공을 약속하는 자극적인 제목, 근거 없는 성공 신화, 반복되는 공식들 — Whiteboard 에디터들은 이런 책들을 철저히 배제한다. 대신 우리가 주목하는 것은 심리학과 행동과학에 기반한 검증된 원칙, 저자 자신의 실패와 성공을 솔직하게 담은 1인칭의 목소리, 그리고 10년 후에도 유효한 본질적인 통찰이다.
...
Whiteboard의 자기계발 큐레이션은 '할 수 있다'는 구호 대신 '어떻게 할 것인가'에 대한 구체적인 해답을 담은 도서들로 구성된다. 동기부여 강연의 흥분이 하루를 못 가는 것처럼, 자기계발서의 감동도 책장을 덮는 순간 사라진다면 아무 의미가 없다. Whiteboard가 선별한 책들은 읽는 동안이 아니라 읽고 난 후에 진짜 효과가 나타난다.`
    },
    ECONOMY: {
        title: "경제 — Whiteboard의 관점",
        body: `돈에 대해 배운 적이 있는가? 학교에서는 수학을 가르쳐도 돈의 심리학은 가르치지 않는다. 경제 뉴스는 매일 쏟아지지만 그 정보들이 내 통장을 두껍게 만들어주지는 않는다. Whiteboard가 경제 카테고리에서 가장 중요하게 여기는 것은 '지식'이 아니라 '태도'다.
...
Whiteboard 에디터들은 재테크 공식을 알려주는 책보다 돈과 인간의 관계를 깊이 탐구하는 책에 더 큰 가치를 부여한다. 매년 새로운 투자 트렌드가 등장하고 사라지지만, 탐욕과 공포라는 인간의 본성은 수천 년째 변하지 않는다. 그 본성을 이해하는 사람이 결국 오랫동안 살아남는다.

이 카테고리에 수록된 도서들은 단기적인 수익 공식이 아닌 경제를 바라보는 장기적인 시각, 복리의 위력과 리스크 관리의 본질, 그리고 소비와 저축에 대한 철학적 기반을 제공한다. 돈을 많이 버는 법보다 돈에 끌려다니지 않는 법을 먼저 배우는 것 — 그것이 Whiteboard 경제 컬렉션의 핵심 명제다.`
    },
    MANAGEMENT: {
        title: "경영 — Whiteboard의 관점",
        body: `리더십이란 무엇인가? 직함이 있는 사람이 리더인가, 아니면 타인의 자발적인 따름을 이끌어내는 사람이 리더인가? Whiteboard가 경영 카테고리에서 묻는 첫 번째 질문이 바로 이것이다. 경영서의 세계는 MBA 교재처럼 딱딱한 이론과 화려한 성공 신화로 가득하다. Whiteboard는 그 사이에서 실제로 조직과 사람을 움직이는 본질적인 원칙들을 발굴한다.
...
Whiteboard의 경영 큐레이션은 CEO를 위한 책이 아니다. 팀 안에서 영향력을 발휘하고 싶은 사람, 자신만의 비즈니스를 꿈꾸는 사람, 혹은 조직의 구조와 인간의 심리를 더 잘 이해하고 싶은 사람 모두를 위한 것이다. 경영이란 결국 사람을 이해하고, 시스템을 설계하고, 불확실성을 관리하는 일이기 때문이다.

위대한 기업은 어떻게 만들어지는가? 좋은 전략이 있는 곳에 좋은 팀이 모이는가, 아니면 좋은 팀이 있는 곳에서 좋은 전략이 나오는가? 이 카테고리의 도서들은 그 질문들에 대한 치열한 탐구의 결과물이다.`
    },
    HUMANITIES: {
        title: "인문 — Whiteboard의 관점",
        body: `인문학은 왜 읽는가? 취업에 도움이 되어서? 교양이 생겨서? Whiteboard 에디터들의 답은 다르다. 인문학은 세상을 다르게 볼 수 있는 언어를 제공하기 때문이다. 우리가 세상을 이해하는 언어의 수가 늘어날수록, 문제를 해결하는 방식의 폭도 그만큼 넓어진다.
...
Whiteboard가 인문 카테고리에서 선택하는 기준은 '오래된 지혜'와 '현재적 적용' 사이의 균형이다. 아리스토텔레스의 철학이나 중세 역사가 지금 이 시대의 우리에게 여전히 유효한 이유는 그것들이 인간의 본성을 다루기 때문이다. 인간의 본성은 스마트폰이 등장했다고 해서 달라지지 않는다.

역사, 철학, 문학, 종교 — 이 카테고리는 인간이 수천 년간 쌓아온 지식의 정수를 담은 책들로 구성된다. 빠르게 변하는 세상일수록 변하지 않는 본질을 아는 것이 가장 강력한 경쟁력이 된다. Whiteboard의 인문 컬렉션은 그 본질을 향한 여정의 지도이다.`
    },
    PSYCHOLOGY: {
        title: "심리 — Whiteboard의 관점",
...
Whiteboard 에디터들은 심리 카테고리를 선정할 때 두 가지 극단을 모두 피한다. 하나는 지나치게 학술적이어서 일상에 적용하기 어려운 책, 다른 하나는 심리학 용어를 남용하는 자기계발서 스타일의 책이다. Whiteboard가 주목하는 것은 연구에 기반하되 삶에 직접 닿는 심리학이다.
...
마음을 돌보는 것은 사치가 아니라 생존 전략이다. Whiteboard의 심리 컬렉션은 나 자신을 더 잘 이해하고, 타인과의 관계에서 더 현명하게 반응하고, 반복되는 감정의 패턴으로부터 자유로워지는 데 필요한 통찰을 담은 도서들로 구성된다. 삶을 바꾸고 싶다면 먼저 자신의 마음을 알아야 한다.`
    }
};
...
export default function CategoryBooks() {
    ...
    return (
        <div className="bg-black text-white font-sans antialiased min-h-screen flex flex-col relative selection:bg-orange-500/30 pb-32">
            <Helmet>
                <title>{categoryInfo.subLabel} 추천 도서 비평 & 인사이트 | Whiteboard(Whiteboard)</title>
                <meta name="description" content={`Whiteboard 에디터가 엄선한 ${categoryInfo.subLabel} 분야 핵심 도서 컬렉션. ${categoryInfo.label} — 단순 목록이 아닌 독창적인 관점으로 선별한 고품질 인사이트를 만나보세요.`} />
            ...
            <main className="flex-grow">
                ...
                {storyBooks.length > 0 && (
                    ...
                    >
                        ...
                        <p className="text-zinc-500 text-[12px] font-bold mb-6 ml-3">Whiteboard 에디터의 독창적인 관점으로 분석한 {categoryInfo.subLabel} 도서</p>
                ...
                <motion.div variants={containerVariants} initial="hidden" animate="visible" className="px-6 space-y-16">
                    ...
                    <section className="space-y-10">
                        <div className="flex items-center justify-between">
                            <h3 className="text-lg font-black tracking-tight">추천 Whiteboard <span className="text-orange-500 ml-1">{categoryBooks.length}</span></h3>
                        </div>
                                    ...
                                    <motion.article key={book.id} variants={itemVariants} className="glass-card rounded-xl p-6 flex flex-col gap-6">
                                        <div className="flex gap-6">
                                            ...
                                            <div className="w-28 h-40 shrink-0 rounded-lg overflow-hidden shadow-2xl relative bg-zinc-800 border border-white/5">
                                                ...
                                                {book.celebritySlug === 'whiteboard_original' && (
