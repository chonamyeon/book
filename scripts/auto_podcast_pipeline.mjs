...
【❗️작성 지침 - 반드시 준수❗️】
1) **내용의 사실성 (Fact-Only)**: 반드시 제공된 [텍스트]의 팩트에만 기반해야 합니다. 책에 없는 내용을 실제 있는 것처럼 꾸며내지 마세요.
2) **철저한 학습**: 제공된 텍스트의 핵심 키워드와 맥락을 깊이 있게 학습하고 이를 대사의 뼈대로 사용하세요. 근거 없는 이야기를 지어내는 것을 엄격히 금지합니다.
3) **위트와 유머**: 직장인들이 공감하고 웃을 수 있는 위트와 유머를 대화 곳곳에 섞어주세요. 유쾌한 톤을 유지하는 것이 중요합니다.
4) **생활 밀착형 상황**: 상황 설정을 사무실에만 가두지 마세요. 출퇴근길, 집안일, 운동, 취미 생활, 친구와의 만남 등 일상 생활 속 다양한 상황을 에피소드에 녹여내세요.

【대본의 정체성】
책 속 아이디어를 소재로 삼아, 제임스와 스텔라가 자신들의 삶에 직접 대입해서 웃고 공감하며 지혜를 나누는 토크 콘텐츠입니다.

【대본 작성 규칙 - ❗️어길 시 시스템 오류 발생❗️】
- ❗ 전체 대화 턴(Turn) 수는 무조건 **정확히 45턴**(제임스 23회, 스텔라 22회)으로 구성하세요. 
- ❗ **인사이트 밀도 극대화**: 서점의 뻔한 줄거리 요약은 5% 이하로 배제하고, 책의 핵심 통찰을 직장인의 실무(보고, 성과, 관계, 번아웃 등)에 날카롭게 대입하세요.
- ❗ 매 턴마다 새로운 시각이나 실천 팁을 제시하여 **2300자 ~ 2500자**를 압축된 인사이트로 꽉 채우세요. (같은 말 반복 절대 엄금)
- ❗ **상황극 중심**: 지식 전달보다 "이거 딱 우리 부장님 이야기네" 식의 생생한 에피소드 비중을 85% 이상으로 구성하세요.
- ❗ 작성 시 내적으로 [1/45], [2/45]... 번호를 매겨 턴 수를 체크하며 완결성 있게 작성하세요 (JSON text 필드에는 번호 제외).
- **[중요]** 마지막 턴에서는 아주 자연스럽게 일상적인 마무리 인사를 나누세요.
- ❗ 제임스(남)가 첫 대사를 시작하며, 스텔라(여)와 완벽하게 교대하세요.
- ❗ "안녕하세요" 같은 상투적인 인사 생략, 바로 일상 속 질문이나 대화로 시작하세요.
...
텍스트: ${bookText.substring(0, 8000)}`;
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
        try {
            ...
            // [안전장치] AI가 60턴 이상 생성한 경우, 자연스러운 마무리 턴을 찾아 거기서 끊음
            if (script.length > 60) {
                console.log(`    ⚠️ [경고] AI가 ${script.length}턴을 생성했습니다. 60턴 부근에서 마무리 지점을 탐색합니다.`);
